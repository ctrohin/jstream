...
# 1. The classic way
x_list = [p.x for p in data if p.y < 10]
...
x_list = extract(data, 5)


# 2. Using jstreams
x_list_stream = stream(data).take_until(lambda p: p.x > 5).to_list()
...
print(x_list_stream)
assert x_list == x_list_stream
print("Less than x 5 items", x_list_stream)
